...
(async()=>{
  const ids=JSON.parse(fs.readFileSync('/app/dedup_ids.json','utf8'));
  if(process.argv[2]!=='--go'){console.log(`预览模式：将删除 ${ids.length} 条重复记录。加 --go 实际执行。`);console.log(ids.join('\n'));return;}
  let ok=0,fail=0;
  for(const id of ids){
    try{
      await jdyAxios.post('/api/v5/app/entry/data/delete',{app_id:JDY.app,entry_id:JDY.entry,data_id:id});
      ok++;
