...
(async()=>{
  for(const sid of ['5974051392723237','5974051392723243','5974051392723248']){
    const r=await reqKm({sid,pageNo:1,pageSize:5});
    const o=(r.list||[])[0];
    if(!o){console.log(sid,'查不到');continue;}
    console.log('sid='+sid+' source='+o.source+' type='+o.type+' tid='+o.tid+' splitSid='+o.splitSid+' payment='+o.payment);
    for(const it of o.orders||[]){console.log('  item.id='+it.id+' title='+it.title+' payAmount='+it.payAmount);}
  }
})();
