...
(async()=>{
  const r=await reqKm({startTime:'2026-08-01 00:00:00',endTime:'2026-08-01 23:59:59',timeType:'created',pageNo:1,pageSize:200});
  const sys=(r.list||[]).filter(o=>o.source==='sys');
  console.log('手动订单(sys) 8/1',sys.length,'单的结构:');
  for(const o of sys.slice(0,5)){
    console.log('\n主单 sid='+o.sid+' tid='+o.tid);
    for(const it of o.orders||[]){
      console.log('  子单 id='+it.id+' oid='+it.oid+' numIid='+it.numIid+' 金额='+it.payment+' 商品='+it.title);
    }
  }
})();
