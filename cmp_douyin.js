...
async function fetchAll(startTs,endTs,timeType){
  ...
  while(true){
    const r=await reqKm({startTime:startTs,endTime:endTs,timeType,pageNo,pageSize:100});
    const list=r.list||[];
    all=all.concat(list);
...
(async()=>{
  const douyin = await fetchAll('2026-08-13 00:00:00','2026-08-13 23:59:59','pay_time');
  const dy = douyin.filter(o=>o.source==='fxg'||o.source==='douyin');
  // 抖音应付金额口径：子单 payAmount 求和
  let totalPay=0; const sids=new Set(); const tids=new Set();
  for(const o of dy){
    sids.add(o.sid); tids.add(o.tid);
    for(const it of o.orders||[]){
      totalPay += Number(it.payAmount)||Number(it.payment)||0;
    }
  }
  console.log('=== 接口 pay_time=8/13 抖音 ===');
  console.log('sid数:', sids.size);
  console.log('tid数:', tids.size);
