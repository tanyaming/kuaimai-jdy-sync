...
(async()=>{
  ...
  console.log('接口 pay_time=8/13 全 source 分布:', JSON.stringify(bySrc));
  console.log('总 sid 数:', all.length);
  // 抖音(fxg)+手动(sys) 合计
  const fxg=all.filter(o=>o.source==='fxg').length;
  const sys=all.filter(o=>o.source==='sys').length;
  console.log('fxg(抖音):', fxg, ' sys(手动):', sys, ' 合计:', fxg+sys);
})();
